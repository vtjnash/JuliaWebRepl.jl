...
cd %THIS_SCRIPT%..
call bin\julia-release-webserver.exe -p 2001 %JULIA_HOME%julia-release-basic.exe

echo Killing nginx... (this can take a few seconds)
for /F "delims=" %%a in (%THIS_SCRIPT%..\logs\nginx.pid) do taskkill /f /t /pid %%a
sleep 1
echo Exiting...
endlocal
popd
